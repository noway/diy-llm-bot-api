...
async function postGenerateChatCompletionStreaming(req: http.IncomingMessage, res: http.ServerResponse, reqBody: string) {
  try {
    ...
    if (apiType === 'chat') {
      if ((model === "gpt-4" || model === "gpt-4.1" || model === "gpt-4.1-mini" || model === "gpt-4.1-nano" || model === "gpt-5" || model === "o1-preview" || model === "o1-mini") && !timeSafeCompare(authKey ?? "", secrets.AUTH_KEY ?? "")) {
