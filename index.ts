...
async function postGenerateChatCompletionStreaming(req: http.IncomingMessage, res: http.ServerResponse, reqBody: string) {
  try {
    ...
    if (model === "gpt-3.5-turbo" || model === "gpt-4" || model === "gpt-4-1106-preview" || model === "gpt-4o-mini" || model === "gpt-4o" || isMixtral || isLlama3_405b || isLlama3_70b || isOpus || isMistralLarge) {
      if (model === "gpt-4" && authKey !== process.env.AUTH_KEY) {
        throw new Error("Invalid auth key");
      }
      const chatMessages = [
        ...(!(isMixtral || isLlama3_405b || isLlama3_70b || isOpus || isMistralLarge) ? [{
          role: "system",
      ...
      const options = {
        ...
        headers: {
          ...
          Authorization: `Bearer ${isMixtral || isLlama3_405b ? process.env.DEEPINFRA_BEARER_TOKEN : ((isOpus || isMistralLarge) ? process.env.OPENROUTER_BEARER_TOKEN : (isLlama3_70b ? process.env.TOGETHER_BEARER_TOKEN : BEARER_TOKEN))}`,
        },
        body: JSON.stringify({
          model,
      ...
      const response = await fetch(
        isMixtral || isLlama3_405b ? "https://api.deepinfra.com/v1/openai/chat/completions" : ((isOpus || isMistralLarge) ? "https://openrouter.ai/api/v1/chat/completions" : (isLlama3_70b ? "https://api.together.xyz/v1/chat/completions" : "https://api.openai.com/v1/chat/completions")),
        options
      );
      if (!response.ok) {
